 ...
 */
public final class Sts implements BasicCommand<Sts.Parameters> {

    @CommandRegistration(name = "sts")
    public static void main(String[] args) {
        BasicCliLauncher.run(args, Parser::new, Sts::new, o -> o.so);
    }
    ...
    public void exec(Parameters params) throws Exception {
        ...
        List<InformationSet> output = StsTool.getDefault().create(input, params.spec);
    ...
    static final class Parser extends JOptSimpleArgsParser<Parameters> {
        ...
        private final ComposedOptionSpec<StsTool.Options> spec = new StsOptionsSpec(parser);
    ...
    @NbBundle.Messages(
            {
        "sts.seas=Seasonal model",
            })
    private static final class StsOptionsSpec implements ComposedOptionSpec<StsTool.Options> {

        private final OptionSpec<SeasonalModel> seas;
        public StsOptionsSpec(OptionParser p) {
            this.seas = p
                    .accepts("seas", Bundle.sts_seas())
                    .withRequiredArg()
                    .ofType(SeasonalModel.class)
                    .defaultsTo(SeasonalModel.Trigonometric);
        }

        @Override
        public StsTool.Options value(OptionSet o) {
            return new StsTool.Options(o.valueOf(seas));
        }
    }
}
